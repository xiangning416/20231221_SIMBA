...
class MERFISHDataSet(ImageDataSet):
    ...
    def _import_positions_from_metadata(self):
        ...
        df = pandas.read_csv(os.sep.join([self.dataHome, 'offsets.csv']), header=None) #assumes that the offset data is stored in a csv file with this name in the datahome folder with the raw data 
        positionData= df.values #comment this and the above line out 

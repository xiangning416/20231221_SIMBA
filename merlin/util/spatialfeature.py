...
class SpatialFeature(object):
    ...
    def __init__(self, boundaryList: List[List[geometry.Polygon]], fov: int,
                 ...
                 transcriptList: List[pandas.DataFrame] = None, 
                 label: int = -1) -> None:
        ...
        if transcriptList is not None:
            self._transcriptList = transcriptList
        else:
            self._transcriptList = []
    ...
    def get_transcript_list(self) -> List[pandas.DataFrame]:
        return self._transcriptList
    
    def set_transcript_list(self, transcriptList: List[pandas.DataFrame]) -> None:
        self._transcriptList.append(transcriptList)
    ...
    def contains_point(self, point: geometry.Point, zIndex: int) -> bool:
        ...
        """
        zIndexIndex = self.get_z_coordinates().tolist().index(zIndex)
        for boundaryElement in self.get_boundaries()[zIndexIndex]:
    ...
    def contains_positions(self, positionList: np.ndarray) -> np.ndarray:
        ...
        # print(self.get_z_coordinates())
        if len(bounding_box) != 4:
            return containmentList

        for zIndex in self.get_z_coordinates():
...
class HDF5SpatialFeatureDB(SpatialFeatureDB):
    ...
    def _save_feature_to_hdf5_group(h5Group: h5py.Group,
        ...
        featureGroup.attrs['id'] = np.string_(str(feature.get_feature_id()))
        # print(np.string_(str(feature.get_feature_id())).decode())
        ...
        featureGroup['transcript_coords'] = feature.get_transcript_list()
        ...
        #             geometryGroup, b)
        print(len(feature.get_boundaries()), len(feature.get_z_coordinates()))
        for i, bSet in zip(feature.get_z_coordinates(), feature.get_boundaries()):
    ...
    def _load_feature_from_hdf5_group(h5Group):
        zIndcies = [x for x in h5Group.keys() if x.startswith('zIndex_')]
        boundaryList = []
        for z in range(len(zIndcies)):
            zBoundaryList = []
            zGroup = h5Group[zIndcies[z]]
    ...
    def write_feature_transcripts(self, features: List[SpatialFeature], fov= int) -> None:
